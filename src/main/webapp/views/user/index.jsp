    ...
    <article class="col s5">
        ...
        <security:authorize access="hasAnyRole('NUTRITIONIST','USER')">
            ...
            <jstl:if test="${isFollowing!=true}">
                <a class="button indigo" href="/user/follow.do?userId=${user.id}"><spring:message code="follow"/> </a>
            </jstl:if>
            <jstl:if test="${isFollowing==true}">
                <a class="button indigo" href="/user/unfollow.do?userId=${user.id}"><spring:message code="unfollow"/> </a>
            </jstl:if>
            </jstl:if>
        </security:authorize>
    </article>
